define("./workbox-4c1ac5ce.js",['exports'], function (exports) { 'use strict';
    ...
    */
    ...
    async function executeQuotaErrorCallbacks() {
      {
        logger$2.log(`About to run ${quotaErrorCallbacks$1.size} ` + `callbacks to clean up caches.`);
      }
    ...
    */
    ...
    const _getEffectiveRequest = async ({
      ...
      for (const plugin of cacheKeyWillBeUsedPlugins) {
        ...
        {
          finalAssertExports$2.isInstance(effectiveRequest, Request, {
            moduleName: 'Plugin',
    ...
    const _isResponseSafeToCache = async ({
      ...
      for (const plugin of plugins) {
        ...
        in plugin) {
          ...
          {
            if (responseToCache) {
              finalAssertExports$2.isInstance(responseToCache, Response, {
                moduleName: 'Plugin',
          ...
          if (responseToCache) {
            if (responseToCache.status !== 200) {
              if (responseToCache.status === 0) {
                logger$2.warn(`The response for '${request.url}' is an opaque ` + `response. The caching strategy that you're using will not ` + `cache opaque responses by default.`);
              } else {
                logger$2.debug(`The response for '${request.url}' returned ` + `a status code of '${response.status}' and won't be cached as a ` + `result.`);
              }
            }
          }
        }
    ...
    const matchWrapper = async ({
      ...
      for (const plugin of plugins) {
        ...
        in plugin) {
          ...
          {
            if (cachedResponse) {
              finalAssertExports$2.isInstance(cachedResponse, Response, {
                moduleName: 'Plugin',
    ...
    }) => {
      {
        if (request.method && request.method !== 'GET') {
          throw new WorkboxError$2('attempt-to-cache-non-get-request', {
            url: getFriendlyURL$2(request.url),
      ...
      try {
        ...
        for (const plugin of plugins) {
          ...
          in plugin) {
            ...
            if ("development" !== 'production') {
              if (fetchResponse) {
                finalAssertExports$2.isInstance(fetchResponse, Response, {
                  moduleName: 'Plugin',
    ...
    */
    const messages$3 = {
      strategyStart: (strategyName, request) => `Using ${strategyName} to respond to '${getFriendlyURL$2(request.url)}'`,
      printFinalResponse: response => {
        if (response) {
          logger$2.groupCollapsed(`View the final response here.`);
          logger$2.log(response || '[No response returned]');
          logger$2.groupEnd();
        }
      }
    };
    ...
    */
    ...
    class NetworkFirst {
       ...
       */
      constructor(options = {}) {
        this._cacheName = cacheNames$1.getRuntimeName(options.cacheName);
...

        {
          if (this._networkTimeoutSeconds) {
            finalAssertExports$2.isType(this._networkTimeoutSeconds, 'number', {
      ...
      async handle({
        ...
        {
          logger$2.groupCollapsed(messages$3.strategyStart('NetworkFirst', request));

          for (const log of logs) {
            logger$2.log(log);
          }

          messages$3.printFinalResponse(response);
          logger$2.groupEnd();
        }
        ...
        if (error || !response) {
          ...
          if (event) {
            try {
              ...
              {
                logger$2.warn(`Unable to ensure service worker stays alive when ` + `updating cache for '${getFriendlyURL$2(request.url)}'.`);
              }
            }
          }
        }
    ...
    */
    ...
    class CacheFirst {
      ...
      async handle({
        ...
        {
          logger$2.groupCollapsed(messages$3.strategyStart('CacheFirst', request));
    ...
    */
    ...
    class StaleWhileRevalidate {
      ...
      async handle({
        ...
        {
          logger$2.groupCollapsed(messages$3.strategyStart('StaleWhileRevalidate', request));
...
//# sourceMappingURL=workbox-4c1ac5ce.js.map
